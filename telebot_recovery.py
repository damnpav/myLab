...
token_path = r'/home/jet/PycharmProjects/credentials/bot_token.txt'
bot_token = open(token_path).readlines()[0].replace('\n', '')
home_path = r'/home/jet/PycharmProjects/myLab/telega_hosting/'
...
def print_out_model(shares, quantity, username):
    ...
    return_dict = {'return_str': return_str, 'pie_path': home_path+pie_path, 'corr_path': home_path+corr_path}
    return return_dict
...
def search_handler(message):
    ...
    if type(return_str) == str:
        ...
        bot.send_photo(chat_id=chat_id, photo=open(home_path+found_path, 'rb'), caption='Found companies',
                       parse_mode='HTML')
...
def log_message(message, type):
    ...
    query = f"INSERT INTO LOGGING VALUES ('{dt.now().strftime('%H:%M:%S %d-%m-%Y')}', '{message.chat.id}', "\
            f"'{message.from_user.username}', '{message.text}', '{type}')"
    print(query)
